<?php
    ...
    public function returnLendingHistory($start, $end, $limit = null) {
      return $this->query(
		...
		public function get_historic_trade_history($pair, $start, $end) {
      $trades = $this->retrieveJSON($this->public_url.'?command=returnTradeHistory&currencyPair='.strtoupper($pair).'&start='.$start.'&end='.$end);
      return $trades;
    }

		public function returnChartData($pair) {
			$trades = $this->retrieveJSON($this->public_url.'?command=returnChartData&currencyPair='.strtoupper($pair));
			return $trades;
		}

    public function returnHistoricChartData($pair, $start, $end) {
      $trades = $this->retrieveJSON($this->public_url.'?command=returnChartData&currencyPair='.strtoupper($pair).'&start='.$start.'&end='.$end.'&period=14400');
      return $trades;
    }
